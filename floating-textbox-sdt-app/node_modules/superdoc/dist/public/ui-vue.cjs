...
const require_create_super_doc_ui = require("../chunks/create-super-doc-ui-Bg4nEED3.cjs");
...
function provideSuperDocUI() {
	const ui = (0, vue.shallowRef)(null);
	const host = (0, vue.shallowRef)(null);
	...
	*/
	...
	const setSuperDoc = (superdoc) => {
		if (disposed) return;
		const rawSuperDoc = (0, vue.toRaw)(superdoc);
		if (retiredHosts.has(rawSuperDoc)) return;
		const hostUi = rawSuperDoc.ui;
		disposeOwnedUi();
		if (hostUi) ui.value = hostUi;
		else {
			const created = require_create_super_doc_ui.createSuperDocUI({ superdoc: rawSuperDoc });
			ownedUi = created;
			ui.value = created;
		}
	...
	const clearSuperDoc = (expectedHost) => {
		...
		const rawExpected = (0, vue.toRaw)(expectedHost);
	...
	(0, vue.provide)(SUPERDOC_UI_KEY, binding);
	return binding;
}
function useBinding() {
	const binding = (0, vue.inject)(SUPERDOC_UI_KEY, null);
...
function useSuperDocSlice(pick, initial) {
	...
	const value = (0, vue.shallowRef)(initial);
	(0, vue.watch)(ui, (current, _previous, onCleanup) => {
		if (!current) {
			value.value = initial;
			return;
		}
		const source = require_slice_source.toSliceSource(pick(current));
...
function useSuperDocCommand(id) {
	const ui = useSuperDocUI();
	const state = (0, vue.shallowRef)(EMPTY_COMMAND);
	(0, vue.watch)([ui, () => (0, vue.toValue)(id)], ([currentUi, currentId], _previous, onCleanup) => {
		if (!currentUi) {
	...
	const execute = (payload) => {
		...
		return currentUi ? currentUi.commands.execute((0, vue.toValue)(id), payload) : false;
	};
	const executeAsync = (payload) => {
		const currentUi = ui.value;
		return currentUi ? currentUi.commands.executeAsync((0, vue.toValue)(id), payload) : Promise.resolve(false);
	};
	return {
		state,
		enabled: (0, vue.computed)(() => state.value.enabled),
		active: (0, vue.computed)(() => state.value.active),
		supported: (0, vue.computed)(() => state.value.supported),
		execute,
		executeAsync
	};
}
...
//#endregion
exports.provideSuperDocUI = provideSuperDocUI;
exports.useClearSuperDoc = useClearSuperDoc;
exports.useSetSuperDoc = useSetSuperDoc;
exports.useSuperDocCommand = useSuperDocCommand;
exports.useSuperDocComments = useSuperDocComments;
exports.useSuperDocContentControls = useSuperDocContentControls;
exports.useSuperDocDocument = useSuperDocDocument;
...
exports.useSuperDocTrackChanges = useSuperDocTrackChanges;
exports.useSuperDocUI = useSuperDocUI;
exports.useSuperDocZoom = useSuperDocZoom;
