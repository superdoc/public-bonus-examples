...
var InteractionRecorder = class {
	constructor(config, version) {
		this.version = version;
		this.sessionId = `interaction-${++nextSession}`;
		this.slots = [];
		this.head = 0;
		this.count = 0;
		this.sequence = 0;
		...
		this.maxEvents = limit(own(history, "maxEvents"), 500, 1e4);
		this.maxBytes = limit(own(history, "maxBytes"), 1048576, 16777216);
	}
	record(type, read) {
		...
		try {
			...
			const project = (value, depth = 0) => {
				...
				if (Array.isArray(value)) {
					...
					const result = [];
					for (let i = 0; i < length && remaining > 0; i++) result.push(project(own(value, String(i)), depth + 1) ?? null);
					return result;
				}
				const result = {};
				for (const key of this.captureContent ? [...fields, ...contentFields] : fields) {
					...
					if (copied !== void 0) result[key] = copied;
				}
				return result;
			};
			...
			} else this.lastSelection = void 0;
			const event = {
	...
	action() {
		return this.enabled && !this.closed ? `${this.sessionId}:${++this.actions}` : void 0;
	}
	snapshot() {
		const events = [];
		try {
	...
	attach(root) {
		...
		for (const type of [
			...
			const listener = (event) => this.record(`input:${type}`, () => {
				const input = event;
				if (event.type === "keydown") {
					...
					return {
						inputKind: [
							...
							"ArrowUp",
							"ArrowDown"
						].includes(keyboard.key) ? keyboard.key : "key",
						dataLength: keyboard.key.length === 1 ? 1 : void 0,
...
function createInteractionHistory(owner, config, version, root) {
	try {
...
function recordInteraction(owner, type, read) {
	try {
		if (owner) recorders.get(owner)?.record(type, read);
	} catch {}
}
function beginInteraction(owner, type, read) {
	try {
		const recorder = recorders.get(owner);
		const actionId = recorder?.action();
		if (actionId) recorder?.record(type, () => ({
			...read(),
			actionId
		}));
		return actionId;
...
function closeInteractionHistory(owner) {
	try {
...
* undo/redo.
*/
function collectV2InvalidatedCommentIds(event) {
	const removedIds = /* @__PURE__ */ new Set();
	if (event?.type !== "mutation:committed") return removedIds;
	const payload = event.origin === "history" ? event.result : event.receipt;
	if (!Array.isArray(payload?.invalidatedRefs)) return removedIds;
	for (const ref of payload.invalidatedRefs) if (ref?.kind === "entity" && ref.entityType === "comment" && typeof ref.entityId === "string" && ref.entityId) removedIds.add(ref.entityId);
	return removedIds;
}
function applyV2CommentInvalidationFromMutation({ event, commentsStore, superdoc, documentId } = {}) {
	const removedIds = collectV2InvalidatedCommentIds(event);
	if (removedIds.size === 0) return {
		ok: true,
		removedIds: []
	};
	return commentsStore?.dropCommentsFromMutationImpact?.({
		superdoc,
		documentId,
		removedIds
	}) ?? {
...
function createSuperDocUI(options) {
	...
	const contextMenu = {
		open: () => {
			...
			if (!surface || typeof surface !== "object" || typeof surface.open !== "function") return {
				ok: false,
			...
			try {
				...
				if (result && typeof result === "object" && result.ok === false) return {
					ok: false,
					reason: coerceSuperDocUIReason(result.reason, SUPERDOC_UI_REASONS.operationUnavailable)
				};
				return { ok: true };
			} catch {
				return {
	...
	*/
	...
	let pendingPostPaintSelectionRefresh = null;
	...
	const resetPostPaintSelectionRefresh = () => {
	...
	const schedulePostPaintSelectionRefresh = () => {
		const editor = getEditor();
		const readiness = editor?.documentMutationReadiness;
		if (!editor || typeof readiness?.whenPainted !== "function") return false;
		pendingPostPaintSelectionRefresh = {
			editor,
			readiness,
			afterEpoch: typeof readiness.getRenderEpoch === "function" ? safeCall(() => readiness.getRenderEpoch.call(readiness), null) : null
		};
	...
	*/
	...
	const trackedChangeDecisionDisabledReason = () => {
		if (readDocumentMode() === "viewing") return SUPERDOC_UI_REASONS.documentReadonly;
		const trackedChanges = ((superdoc && typeof superdoc === "object" ? superdoc : null)?.interactionConfig)?.trackedChanges;
		if (trackedChanges && typeof trackedChanges.allowDecisions === "boolean") return trackedChanges.allowDecisions ? void 0 : SUPERDOC_UI_REASONS.trackedChangeDecisionsDisabled;
		return commentsAreReadOnly() ? SUPERDOC_UI_REASONS.documentReadonly : void 0;
	};
	/**
	...
	*/
	...
	const commentMutationsAreReadOnly = () => commentsAreReadOnly();
	const trackedChangeDecisionsAreDisabled = () => trackedChangeDecisionDisabledReason() != null;
	...
	*/
	...
	let settledActiveContentControlsToken = null;
	let settledActiveContentControls = [];
	const retainSettledActiveContentControls = (items, status) => {
		const token = contentToken();
		if (status === "ready") {
		...
		if (status === "pending" && settledActiveContentControlsToken === token) return {
			items: settledActiveContentControls,
			status: "stale"
		};
		return {
			items,
	...
	const computeContentControls = (selection) => {
		...
		const items = [...catalogItems.map((item) => activeById.get(item.id) ?? item), ...activeItems.filter((item) => !catalogIds.has(item.id))];
	...
	*/
	...
	const computeActiveContentControls = (ccApi, selection) => {
		const { rows, status } = computeActiveContentControlLockModes(ccApi, selection);
		const explicitTarget = selection.selectionTarget;
		const selectionTarget = selection.target;
		let selectionAnchor = explicitTarget?.start;
		if (explicitTarget) {
			if (explicitTarget.kind !== "selection" || selectionAnchor?.kind !== "text" || typeof selectionAnchor.blockId !== "string" || typeof selectionAnchor.offset !== "number") return retainSettledActiveContentControls([], status);
		} else {
			const firstSegment = selectionTarget?.kind === "text" && Array.isArray(selectionTarget.segments) ? selectionTarget.segments[0] : void 0;
			const firstRange = firstSegment?.range;
			if (typeof firstSegment?.blockId !== "string" || typeof firstRange?.start !== "number") return retainSettledActiveContentControls([], status);
			selectionAnchor = {
				kind: "text",
		...
		const visibleSegment = (selectionTarget?.coordinateSpace !== "tracked" && Array.isArray(selectionTarget?.segments) ? selectionTarget.segments : []).find((segment) => segment.blockId === selectionAnchor.blockId);
		const visibleRange = visibleSegment?.range;
		const visibleStart = visibleSegment && typeof visibleSegment.blockId === "string" && typeof visibleRange?.start === "number" ? {
			kind: "text",
			blockId: visibleSegment.blockId,
			offset: visibleRange.start,
			story: selectionTarget?.story ?? selectionAnchor.story
		} : null;
		const targetCoordinateSpace = (explicitTarget ? explicitTarget.coordinateSpace : selectionTarget?.coordinateSpace) === "tracked" ? "tracked" : "visible";
		...
		const selectionStartIndex = selectedBlockIds.indexOf((selectionStart ?? selectionAnchor).blockId);
		const activeItems = rows.map((item, index) => ({
			item,
			index
		})).filter(({ item }) => {
			const itemTarget = item.selectionTarget;
			...
			const itemStart = itemTarget?.start;
			const itemEnd = itemTarget?.end;
			if (itemTarget?.kind !== "selection" || itemStart?.kind !== "text" || itemEnd?.kind !== "text" || typeof itemStart.blockId !== "string" || typeof itemEnd.blockId !== "string" || typeof itemStart.offset !== "number" || typeof itemEnd.offset !== "number") return false;
			if (storyLocatorSignature(itemTarget.story ?? itemStart.story ?? itemEnd.story) !== storyLocatorSignature(explicitTarget?.story ?? start.story)) return false;
			...
			if (itemStartIndex !== -1 && itemEndIndex !== -1) return selectionStartIndex > Math.min(itemStartIndex, itemEndIndex) && selectionStartIndex < Math.max(itemStartIndex, itemEndIndex);
			...
			const leftSameBlock = leftTarget.start.kind === "text" && leftTarget.end.kind === "text" && leftTarget.start.blockId === leftTarget.end.blockId;
			const rightSameBlock = rightTarget.start.kind === "text" && rightTarget.end.kind === "text" && rightTarget.start.blockId === rightTarget.end.blockId;
			...
			if (leftSameBlock && rightSameBlock && leftTarget.start.kind === "text" && leftTarget.end.kind === "text" && rightTarget.start.kind === "text" && rightTarget.end.kind === "text") {
	...
	*/
	...
	const trackDecisionReason = (command, supported, disabledReason, selection, doc) => {
	...
	const computeCommandState = (id, doc, selection, projectedInlineValues, lockModesById = /* @__PURE__ */ new Map()) => {
		...
		if (trackCommand) {
			...
			const reason = bulkTrackDecisionBlockedReason(trackCommand, tcApi) ?? trackDecisionReason(trackCommand, supported, trackedChangeDecisionDisabledReason(), selection, doc) ?? (trackCommand.scope === "all" ? void 0 : trackedChangeDecisionPermissionReason(trackCommand.kind, selection.activeChangeIds));
			return normalizeCommandState({
				enabled: reason == null,
				active: false,
				supported,
	...
	*/
	...
	const styleCatalogReadKey = (input) => `styles:catalog:${JSON.stringify(input ?? {})}`;
	/**
	...
	*/
	const readStyleCatalogLive = (input) => {
		...
		return readAsync(styleCatalogReadKey(input), contentToken(), () => stylesApi.getCatalog(input), (raw) => raw && typeof raw === "object" ? raw : null);
	};
	...
	*/
	...
	const caretSelectionNeedsProjectionPaint = () => {
		const host = getHost();
		const read = host?.readLiveSelectionSyncSnapshot;
		if (typeof read !== "function") return false;
		const info = normalizeSelectionInfo(safeCall(() => read.call(host), null));
		...
		if (previousCaret && nextCaret && previousCaret.blockId === nextCaret.blockId && storyLocatorSignature(previousCaret.story) === storyLocatorSignature(nextCaret.story)) return false;
		...
		const hadSize = typeof sizeValue === "string" && sizeValue.length > 0 || typeof sizeValue === "number" && Number.isFinite(sizeValue);
		return hadFamily && projected.fontFamily === void 0 || hadSize && projected.fontSize === void 0;
	};
	const syncHostSelectionSubscription = () => {
		...
		try {
			const unsubscribe = next.subscribe((snapshot) => {
				if (foregroundMutationActive() && caretSelectionNeedsProjectionPaint() && schedulePostPaintSelectionRefresh()) return;
				selectionEpoch += 1;
	...
	*/
	...
	const executeCommandInternal = (id, payload, context) => {
		pendingCommandSettlement = null;
	...
	const executeCommand = (id, payload, context) => {
		...
		const settled = (value) => recordInteraction(superdoc, "command:settled", () => ({
			actionId,
			commandId: id,
		...
		if (result === false) settled(result);
			...
			lastCommandSettlement.then(settled, () => settled(false));
		} catch {}
		return result;
	};
	...
	*/
	const getSelectionApplyHelper = () => {
		...
		const editingHelper = (handles?.editing)?.selectionTargets;
		if (typeof editingHelper?.apply === "function") return { helper: editingHelper };
		const reviewHelper = handles?.selection;
		if (typeof reviewHelper?.apply === "function") return { helper: reviewHelper };
	...
	const readHostSelectionTarget = () => {
		...
		const rendered = ((handles?.editing)?.selection)?.toSelectionTarget?.() ?? (handles?.selection)?.toSelectionTarget?.();
		if (!rendered || typeof rendered !== "object") return null;
		if (rendered.kind !== "ok") return null;
	...
	*/
	const commentWriteRefusal = () => commentMutationsAreReadOnly() ? failedReceipt("Comments are read-only.", "DOCUMENT_READONLY") : null;
	...
	const comments = {
		...
		createFromSelection: (input) => {
			...
			if (!target) return failedReceipt("A range selection is required to comment.", "NO_SELECTION");
			const fallback = failedReceipt("comments.create failed.");
			return safeCall(() => settleWorkflowReceipt(op.call(commentsApi, {
				target,
				text: input.text
	...
	*/
	...
	let contentControlHighlightHost = null;
	const clearContentControlHighlight = () => {
		contentControlHighlightGeneration += 1;
		const host = contentControlHighlightHost ?? getHost();
		contentControlHighlightHost = null;
		safeCall(() => (host?.clearContentControlHighlight)?.call(host), void 0);
	};
	...
	const contentControls = {
		...
		highlight: async (input) => {
			...
			const host = getHost();
			const editor = getEditor();
			const controls = getDoc()?.contentControls;
			if (disposed || !host || typeof host.highlightContentControl !== "function" || typeof controls?.list !== "function") return {
				success: false,
				reason: "not-ready"
			};
			const request = ++contentControlHighlightGeneration;
			contentControlHighlightHost = host;
			safeCall(() => host.cancelContentControlHighlightRequest?.call(host), void 0);
			const cancelled = () => disposed || request !== contentControlHighlightGeneration || editor !== getEditor();
			try {
				const catalog = await controls.list.call(controls);
				if (cancelled()) return {
					success: false,
					reason: "cancelled"
				};
				const control = (catalog?.items)?.find((item) => item.id === id);
				if (!control) return {
					success: false,
					reason: "not-found"
				};
				const target = readSelectionTarget(control);
				if (!target || control.isEmpty) return {
					success: false,
					reason: "not-reachable"
				};
				const success = await host.highlightContentControl.call(host, id, target);
				if (cancelled()) return {
					success: false,
					reason: "cancelled"
				};
				...
				return {
					success: false,
					reason: cancelled() ? "cancelled" : "not-reachable"
				};
			}
		},
		get: ((input) => {
	...
	const documentHandle = {
		...
		replaceDocument: async (file) => {
			...
			if (typeof host.replaceDocument !== "function") return {
				ok: false,
				reason: SUPERDOC_UI_REASONS.operationUnavailable
			};
	...
	*/
	...
	const setSearchState = (patch) => {
		...
		if (includeTrackedDeletions !== void 0) {
	...
	*/
	...
	const search = {
		...
		search: (query, options) => search.find(query, options),
		next: () => {
			const host = getHostSearch();
...
//#endregion
export { getV2TrackedChangeMutationImpact as a, createInteractionHistory as c, applyV2CommentInvalidationFromMutation as i, recordInteraction as l, shallowEqual as n, DOM_CLASS_NAMES as o, BUILT_IN_COMMAND_IDS as r, closeInteractionHistory as s, createSuperDocUI as t };
