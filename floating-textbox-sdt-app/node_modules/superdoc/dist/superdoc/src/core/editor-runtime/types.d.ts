 ...
 */
export type EditorRuntimeState = 'opening' | 'blocked' | 'review-ready' | 'editing-ready' | 'saving' | 'disposed' | 'failed';
/**
...
export type EditorRuntimeCommand = {
    kind: 'text.insert';
    text: string;
    at?: EditorRuntimePositionToken;
} | {
    kind: 'text.replace';
    text: string;
    range?: EditorRuntimePositionToken;
} | {
    ...
    kind: 'formatting.applyMark';
    mark: string;
    value?: RuntimeJsonValue;
} | {
    kind: 'formatting.applyParagraph';
    ...
    kind: 'comments.resolve';
    commentId: string;
} | {
    kind: 'comments.reopen';
    commentId: string;
} | {
    ...
    kind: 'trackedChanges.accept';
    id: string;
} | {
    kind: 'trackedChanges.reject';
    id: string;
} | {
    ...
    kind: 'trackedChanges.setAuthoringMode';
    mode: 'direct' | 'tracked';
};
 ...
 */
export type EditorRuntimeNoopReason = 'nothing-to-undo' | 'nothing-to-redo' | 'no-effect' | 'already-applied' | 'empty-selection';
/**
 ...
 */
export type EditorRuntimeRejectionCode = 'runtime-not-ready' | 'host-saving' | 'capability-unsupported' | 'command-unsupported' | 'selection-unsupported' | 'target-unsupported' | 'document-readonly' | 'command-failed' | 'stale-position-token' | 'wrong-runtime-token' | 'author-required' | 'selection-invalidated' | 'review-command-unavailable';
/**
...
export type EditorRuntimeCommandResult = {
    ...
    status: 'noop';
    reason: EditorRuntimeNoopReason;
} | {
    status: 'history-noop';
    reason: EditorRuntimeNoopReason;
    result?: unknown;
    ...
    status: 'rejected';
    reason: EditorRuntimeRejectionCode;
    detail?: string;
};
/**
...
export type EditorRuntimeNavigationTarget = {
    kind: 'position';
    position: EditorRuntimePositionToken;
} | {
    kind: 'page';
    page: number;
} | {
...
export type EditorRuntimeEvent = {
    ...
    selection: EditorRuntimeSelectionSnapshot;
} | {
    type: 'capabilities-change';
    capabilities: EditorRuntimeCapabilities;
} | {
    type: 'toolbar-state-change';
    state: EditorRuntimeToolbarState;
} | {
    type: 'layout-change';
    layout: EditorRuntimeLayoutSnapshot;
} | {
    type: 'state-change';
